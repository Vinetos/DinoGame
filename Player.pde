class Player {
  float fitness;
  Genome brain; // "Cerveau" du joueur
  boolean replay = false;// Mode replay ou non

  float unadjustedFitness;
  int lifespan = 0;// Combien de temps le joueur a vécu pour la fitness
  int bestScore = 0;// le score obtenu utilisé pour la relecture
  boolean dead; // mort ?
  int score; // Score actuel
  int gen = 0; // N° de génération

  int genomeInputs = 7; // 7 Entrées
  int genomeOutputs = 3; // 3 Sorties (Actions possibles)

  float[] vision = new float[genomeInputs];// Le tableau d'entrée envoyé au réseau neuronal
  float[] decision = new float[genomeOutputs]; // La sortie du réseau neuronal
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------------
  // Affiche le dino (avec les différentes images en fonction de l'action du joueur (marche, saut, duck...))
  void show() {
    if (duck && posY == 0) { // Accroupi et sur le sol
        image(runCount < 0 ? dinoDuck : dinoDuck1, playerXpos - dinoDuck.width/2, height - groundHeight - (posY + dinoDuck.height));
    } else 
      if (posY == 0) // Sur le sol (cours)
          image(runCount < 0 ? dinoRun1 : dinoRun2, playerXpos - dinoRun1.width/2, height - groundHeight - (posY + dinoRun1.height));
      else // En saut
        image(dinoJump, playerXpos - dinoJump.width/2, height - groundHeight - (posY + dinoJump.height));
    
    // runCount < 0 : Tous les 5 rafraichissements, on change l'image du dino (ce qui le fait "courir")
    runCount++;
    if (runCount > 5) { 
      runCount = -5;
    }
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------------
  // Vérifie les collisions et s'il s'agit d'un replayer, déplace tous les obstacles
  void move() {
    posY += velY;
    if (posY >0) {
      velY -= gravity;
